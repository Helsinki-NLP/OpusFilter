"""Command-line interface for opusfilter-autogen command."""
import argparse
import json
import logging
import os
...
def main(args=None):
    ...
    parser.add_argument('--noisy-percentile', default=0.001, type=float, metavar='FLOAT',
                        help='Proportion of data considered to be noisy; only for percentiles method (default %(default)s)')
    parser.add_argument('--clusters', '-k', default=2, type=int, metavar='INT',
                        help=('Number of clusters for clustering method; try increasing if too much data is clustered '
    ...
    parser.add_argument('--plot', metavar='PATH', default=None, type=str,
                        help=('Create histograms of feature data distributions and a scatter plot of the clustering; '
                              'give path to plot PDF files to, or "-" for interactive plots; only for clustering method'))
    parser.add_argument('--list-defaults', action='store_true', help='List default filters of method to output and quit')
