...
def _check_variables(variables):
    """Check that variable definitions are valid and return number of combinations."""
    lengths = set()
    for key, value in variables.items():
        if not isinstance(value, list):
            raise ValueError(f"Variable {key} does not define a list")
...
def expand_steps_with_variables(steps, common_constants=None):
    ...
    for original_idx, step in enumerate(steps):
        ...
        num_choices = _check_variables(variables)
        if not num_choices:
