...
from opusfilter.autogen_cluster import ThresholdFinder

default_params = {'AlphabetRatioFilter': {},
     'CharacterScoreFilter': {'scripts': ['latin', 'latin']},
     'LanguageIDFilter': {'id_method': 'cld2', 'languages': ['en', 'de']},
...
class TestThresholdFinder(unittest.TestCase):
    ...
    def test_get_default_parameters(self):
        tf = ThresholdFinder([], ['en', 'de'], ['latin', 'latin'], None, None, None, None, None, None)
        self.assertEqual(tf.filter_params, default_params)

    def test_reject_all_parameters(self):
        tf = ThresholdFinder([], ['en', 'de'], ['latin', 'latin'], None, None, None, None, None, None)
        tf.get_parameters([1 for i in range(10)], {k: True for k in default_rejects.keys()})
        self.assertEqual(tf.filter_params, {})

    def test_get_all_parameters(self):
        tf = ThresholdFinder([], ['en', 'de'], ['latin', 'latin'], None, None, None, None, None, None)
        tf.get_parameters([1 for i in range(10)], default_rejects)
        self.assertEqual(tf.filter_params, example_params)

    def test_get_parameters_reject_one_side(self):
        tf = ThresholdFinder([], ['en', 'de'], ['latin', 'latin'], None, None, None, None, None, None)
        ...
        tf.get_parameters([1 for i in range(10)], rejects)
