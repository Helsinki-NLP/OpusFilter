...
class TestAutogen(unittest.TestCase):
    ...
    def _test_filters(self, filters):
        generator = ConfigurationGenerator([self.src_out, self.tgt_out], workdir='work', langs=[self.source, self.target])
    ...
    def test_default_filters(self):
        ...
        self._test_filters(filters)
        
    @unittest.expectedFailure
    def test_percentile_filters(self):
        filtergen = PercentileFilters(files=[self.src_out, self.tgt_out], excluded_percentile=0.05)
        filters = filtergen.get_thresholds()
        self._test_filters(filters)
